...
import {
  ShoppingCart, ChevronRight, User, CalendarDays, MapPin,
    ...
    <div className="min-h-full bg-slate-50">
      ...
      <div className="border-b border-slate-200 bg-white px-6 py-5">
        <div className="flex items-center gap-2 text-sm text-slate-500 mb-3">
          <Link href="/pedidos" className="flex items-center gap-1.5 hover:text-teal-600 transition-colors">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl font-semibold text-slate-900 font-mono">Pedido #{p.numero}</h1>
              ...
              <p className="mt-0.5 text-sm text-slate-500">Creado el {formatDateTime(p.created_at)}</p>
            </div>
          </div>
      ...
      <div className="p-6 space-y-5 max-w-6xl">
        ...
        {p.estado === 'cancelado' && p.notas && (
          <div className="rounded-lg border border-red-200 bg-red-50 p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-red-700 mt-0.5">{p.notas}</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-lg font-bold text-slate-900">{formatCurrency(p.subtotal)}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Descuento</p>
            <p className="mt-1 text-lg font-bold text-red-600">{formatCurrency(p.descuento)}</p>
          </div>
          <div className="rounded-xl border border-teal-200 bg-teal-50 p-4 shadow-sm col-span-2 sm:col-span-1">
            <p className="text-xs font-medium text-teal-600 uppercase tracking-wide">Total</p>
            <p className="mt-1 text-lg font-bold text-teal-700">{formatCurrency(p.total)}</p>
          </div>
        </div>
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="px-5 py-4 space-y-1.5">
                {p.cliente ? (
                  <>
                    <Link href={`/clientes/${p.cliente.id}`} className="font-medium text-teal-600 hover:text-teal-700 text-sm">
                      {p.cliente.nombre}
                    </Link>
                    {p.cliente.rif && <p className="text-xs text-slate-500">RIF: {p.cliente.rif}</p>}
                    {p.cliente.telefono && <p className="text-xs text-slate-500">{p.cliente.telefono}</p>}
                    {p.cliente.direccion && <p className="text-xs text-slate-500">{p.cliente.direccion}</p>}
                  </>
            ...
            <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="divide-y divide-slate-50 px-5">
                ...
                <InfoRow label="Entrega estimada" value={p.fecha_entrega_estimada ? formatDate(p.fecha_entrega_estimada) : null} />
                <InfoRow label="Entrega real" value={(p as any).fecha_entrega_real ? formatDate((p as any).fecha_entrega_real) : null} />
              </div>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="px-5 py-4">
                {p.conductor ? (
                  <>
                    <p className="text-sm font-medium text-slate-900">{p.conductor.nombre}</p>
                    {p.conductor.telefono && <p className="text-xs text-slate-500 mt-0.5">{p.conductor.telefono}</p>}
                  </>
            ...
            {/* Notas (only if not cancelado — shown in banner above) */}
            {p.notas && p.estado !== 'cancelado' && (
              <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
                <div className="border-b border-slate-100 px-5 py-3">
                  <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-900">
          ...
          <div className="lg:col-span-2">
            <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
              <div className="border-b border-slate-100 px-5 py-4 flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-slate-900">Ítems del pedido</h2>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-100 bg-slate-50">
                          ...
                          <th className="px-5 py-3 text-center text-xs font-semibold uppercase tracking-wide text-slate-500">Cant.</th>
                          <th className="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-slate-500">P. Unit.</th>
                          <th className="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-slate-500">Desc.</th>
                          <th className="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-slate-500">Subtotal</th>
                        </tr>
                      </thead>
                  ...
                  <div className="border-t border-slate-200 bg-slate-50 px-5 py-4">
                    <div className="flex flex-col items-end gap-1.5 text-sm">
                      <div className="flex justify-between w-48">
                        ...
                        <span className="font-medium text-slate-900">{formatCurrency(p.subtotal)}</span>
                      </div>
                      {p.descuento > 0 && (
                        <div className="flex justify-between w-48">
                          <span className="text-slate-500">Descuento</span>
                          <span className="font-medium text-red-600">− {formatCurrency(p.descuento)}</span>
                        </div>
                      )}
...
function InfoRow({ label, value }: { label: string; value?: string | null }) {
  return (
    <div className="flex items-start justify-between py-3 gap-4">
      ...
      <span className="text-sm text-slate-900 text-right">{value || <span className="text-slate-400">—</span>}</span>
    </div>
  )
}

...
export async function POST(_req: Request, { params }: RouteContext) {
  try {
    const supabase = createClient()
      ...
      // Pedido nace YA aprobado (Fase 4 — modelo limpio: el pedido es la
      // orden ya aprobada, no requiere "confirmar" ni "aprobar pedido"
      // otra vez). estado_despacho='por_despachar' arranca el lifecycle
      // de envío, separado del estado del pedido.
      ...
      if (pedidoErr && /estado_despacho/i.test(pedidoErr.message || '')) {
        console.warn('[ordenes/aprobar] pedidos.estado_despacho missing — retrying without')

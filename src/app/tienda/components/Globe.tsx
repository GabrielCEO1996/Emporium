...
// ─── Distribution arcs — Chicago HQ + rutas cross-country ────────────────
// Bola más pequeña que el original (1.4 → 1.1) y reposicionada un poco
// más a la derecha. Los arcs son rutas país→país: 8 desde Chicago HQ + 4
// cross-country para que el globo se sienta como red de distribución
// global, no solo "outbound desde Chicago".
const SPHERE_RADIUS = 1.1
const ARC_SEGMENTS = 60
const ARC_LIFT = 0.24
const ARC_DURATION = 4.5

const CHICAGO = { lat: 41.88, lng: -87.63 } // mantenido para el marker HQ
...
const ARC_ROUTES: ReadonlyArray<{
  ...
  // Chicago HQ → destinos
  { name: 'CHI→Miami',       from: CHICAGO, to: { lat:  25.76, lng:  -80.19 } },
  { name: 'CHI→NY',          from: CHICAGO, to: { lat:  40.71, lng:  -74.01 } },
  { name: 'CHI→LA',          from: CHICAGO, to: { lat:  34.05, lng: -118.24 } },
  { name: 'CHI→México',      from: CHICAGO, to: { lat:  19.43, lng:  -99.13 } },
  { name: 'CHI→Bogotá',      from: CHICAGO, to: { lat:   4.71, lng:  -74.07 } },
  { name: 'CHI→São Paulo',   from: CHICAGO, to: { lat: -23.55, lng:  -46.63 } },
  { name: 'CHI→Madrid',      from: CHICAGO, to: { lat:  40.42, lng:   -3.70 } },
  { name: 'CHI→Tokyo',       from: CHICAGO, to: { lat:  35.68, lng:  139.69 } },
  // Cross-country — la red distribuye también entre destinos
  { name: 'Madrid→BA',       from: { lat:  40.42, lng:   -3.70 }, to: { lat: -34.61, lng:  -58.38 } },
  { name: 'London→NY',       from: { lat:  51.51, lng:   -0.13 }, to: { lat:  40.71, lng:  -74.01 } },
  { name: 'Tokyo→Singapore', from: { lat:  35.68, lng:  139.69 }, to: { lat:   1.35, lng:  103.82 } },
  { name: 'Dubai→Mumbai',    from: { lat:  25.20, lng:   55.27 }, to: { lat:  19.08, lng:   72.88 } },
]
...
function EarthScene() {
  ...
  // Chicago marker — solo se usa para el dot + ring del HQ, no para los arcs.
  const chicagoLocal = useMemo(
    () => latLngToVec3(CHICAGO.lat, CHICAGO.lng, SPHERE_RADIUS),
    [],
  )

  // Arcs: cada ruta tiene su propio from/to en lat/lng. La curva se calcula
  // en el espacio local del earth (que rota), así los arcs siguen pegados
  // a la superficie cuando el globo gira.
  const arcs = useMemo(() => {
    return ARC_ROUTES.map((route, i) => {
      const startVec = latLngToVec3(route.from.lat, route.from.lng, SPHERE_RADIUS)
      const endVec = latLngToVec3(route.to.lat, route.to.lng, SPHERE_RADIUS)
      ...
      return {
        endVec,
        points,
        // Distribuir delays a lo largo del ciclo así no salen todos juntos.
        delay: (i / ARC_ROUTES.length) * ARC_DURATION,
      }
    })
  }, [])
  ...
  useFrame((state) => {
    ...
    // Smooth parallax. NO usamos camera.lookAt(earth.position) — eso
    // re-centraba el globo en pantalla y tapaba el texto del hero.
    // Manteniendo la cámara mirando al origen, el earth en world.x=1.9
    // se proyecta naturalmente a la derecha.
    ...
    // Earth rotation — un poco más rápida que antes (0.0008 → 0.0014) para
    // que la red de arcs se sienta viva: el globo da una vuelta completa
    // en ~75 segundos en lugar de ~130s.
    if (earthRef.current) earthRef.current.rotation.y += 0.0014
  ...
  return (
    <>
      {/* Atmosphere — fresnel teal, additive. Radio proporcional al
          nuevo SPHERE_RADIUS=1.1 (1.55 → 1.22, ~+11%). */}
      <mesh ref={atmosphereRef} position={[1.9, 0.05, 0]} material={atmosphereMaterial}>
        <sphereGeometry args={[1.22, 64, 64]} />
      </mesh>

      {/* Earth — desplazado a la derecha para no tapar el texto del hero. */}
      <mesh
        ref={earthRef}
        position={[1.9, 0.05, 0]}
        rotation={initialRot}
